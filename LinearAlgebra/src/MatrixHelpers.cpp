...
namespace LinearAlgebra::Matrix
{
    ...
    Matrix<long double> reduced_row_echelon(const Matrix<T>& m)
    {
        ...
        auto pivots = InternalHelpers::getPivotsFromUpperMatrix(LU_echelon.upper);
        return InternalHelpers::reduceUpperMatrixAndC(LU_echelon.upper, pivots).first;
    }
    ...
    std::vector<Pivot> getPivots(const Matrix<T>& m)
    {
        ...
        return InternalHelpers::getPivotsFromUpperMatrix(LU.upper);
    }
    ...
    {
        const auto LU = factorizeLU_echelon(m);
        return InternalHelpers::rankFromUpperMatrix(LU.upper);
    }
    ...
    std::optional<Solution> solve(const Matrix<T>& A, const Vector::Vector<U>& b)
    {
        ...
        const unsigned int rank = InternalHelpers::rankFromUpperMatrix(LU.upper);

        if (rank == A.rows() && rank == A.cols())
        {
            Solution solution;
            solution.unique         = true;
            solution.uniqueSolution = InternalHelpers::solveFullRankSystem(LU, b);
            return solution;
        }
        else
            return InternalHelpers::solveRankDeficientSystem(LU, rank, b);
    }
