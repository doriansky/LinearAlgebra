...
After generating the TestReport Markdown document, convert it to a PDF file using `mdpdf` or an equivalent Markdown-to-PDF tool. The PDF should be saved alongside the Markdown report with the same base name and a `.pdf` extension (e.g., `TestReport.md` → `TestReport.pdf`). Both files should be saved in the project .claude folder.
